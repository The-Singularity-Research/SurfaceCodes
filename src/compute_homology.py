import numpy
import numpy as np
import numpy.linalg
from src.utilities import permlist_to_tuple, rowSwap, colSwap, scaleCol, scaleRow, colCombine, rowCombine

class homology():
    def __init__(self, d_1, d_2):
        self.d_1 = d_1
        self.d_2 = d_2


    def simultaneous_reduce(self, A, B):
        """
        :param A: a matrix
        :param B: a matrix
        :return: the simultaneous row/column reduction of A and B
        """
        if A.shape[1] != B.shape[0]:
            raise Exception("Matrices have the wrong shape.")

        numRows, numCols = A.shape

        i, j = 0, 0
        while True:
            if i >= numRows or j >= numCols:
                break

            if A[i, j] == 0:
                nonzeroCol = j
                while nonzeroCol < numCols and A[i, nonzeroCol] == 0:
                    nonzeroCol += 1

                if nonzeroCol == numCols:
                    i += 1
                    continue

                colSwap(A, j, nonzeroCol)
                rowSwap(B, j, nonzeroCol)

            pivot = A[i, j]
            scaleCol(A, j, 1.0 / pivot)
            scaleRow(B, j, 1.0 / pivot)

            for otherCol in range(0, numCols):
                if otherCol == j:
                    continue
                if A[i, otherCol] != 0:
                    scaleAmt = -A[i, otherCol]
                    colCombine(A, otherCol, j, scaleAmt)
                    rowCombine(B, j, otherCol, -scaleAmt)

            i += 1;
            j += 1

        return A, B


    def finish_row_reducing(self, B):
        """
        :param B: a matrix
        :return: row reduced form of B
        """
        numRows, numCols = B.shape

        i, j = 0, 0
        while True:
            if i >= numRows or j >= numCols:
                break

            if B[i, j] == 0:
                nonzeroRow = i
                while nonzeroRow < numRows and B[nonzeroRow, j] == 0:
                    nonzeroRow += 1

                if nonzeroRow == numRows:
                    j += 1
                    continue

                rowSwap(B, i, nonzeroRow)

            pivot = B[i, j]
            scaleRow(B, i, 1.0 / pivot)

            for otherRow in range(0, numRows):
                if otherRow == i:
                    continue
                if B[otherRow, j] != 0:
                    scaleAmt = -B[otherRow, j]
                    rowCombine(B, otherRow, i, scaleAmt)

            i += 1;
            j += 1

        return B


    def num_pivot_cols(self, A):
        """
        :param A: a matrix
        :return: the number of pivot columns of A
        """
        z = numpy.zeros(A.shape[0])
        return [numpy.all(A[:, j] == z) for j in range(A.shape[1])].count(False)


    def num_pivot_rows(self, A):
        """
        :param A: a matrix
        :return: the number of pivot rows of A
        """
        z = numpy.zeros(A.shape[1])
        return [numpy.all(A[i, :] == z) for i in range(A.shape[0])].count(False)


    def betti_number(self, d_k, d_kplus1):
        """

        :param d_k: a differential (matrix) in a chain complex
        :param d_kplus1: the differential (matrix) after d_k
        :return: Betti number
        """
        A, B = numpy.copy(d_k), numpy.copy(d_kplus1)
        simultaneous_reduce(A, B)
        finish_row_reducing(B)

        dimKChains = A.shape[1]
        print(dimKChains)
        kernelDim = dimKChains - num_pivot_cols(A)
        print(kernelDim)
        imageDim = num_pivot_rows(B)
        print(imageDim)

        return kernelDim - imageDim
